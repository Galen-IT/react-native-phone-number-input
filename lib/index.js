...
import maskList from './masks.json';
...
export default class PhoneInput extends PureComponent {
  ...
  onChangeText = (text) => {
    const {countryCode} = this.state;
    const maskCurrent = maskList.filter((item) => item.country === countryCode);
    const mask = maskCurrent.length > 0 ? maskCurrent[0] : null;
    const phoneMask = mask
      ? MaskService.toMask('custom', text, {
          mask: mask.mask,
        })
      : text;
    this.setState({number: phoneMask});
    ...
    const number = code.concat(phoneMask.replace(/[\(\)\s-]/g, ''));
    const length = number.length;
    this.setState({lengthNumber: length});
    if (onChangeFormattedText) {
      if (code) {
        onChangeFormattedText(
          text.length > 0 ? `+${code}${phoneMask}` : phoneMask,
        );
      } else {
        onChangeFormattedText(phoneMask);
      }
    }
    if (isValid) {
      isValid(mask && mask.length ? length >= mask.length : true);
    }
  };
        ...
        <View
          ...
          <TouchableOpacity
            ...
            <CountryPicker
              ...
              lang={lang}
              translation={lang === 'ru' ? 'rus' : 'common'}
